...
void sendArpRequest(etherHeader *ether, uint8_t ipFrom[], uint8_t ipTo[]) //Use the same ip address as ipFrom to get a response
{                                                                         //Since the server has linked the IP address to you, you will get a response if someone else has that IP address
    arpPacket *arp = (arpPacket*)ether->data;
    uint8_t i;
    uint8_t localHwAddress[HW_ADD_LENGTH];
