...
//Alex Kowis (DHCP Client - Project 1)
//ID: 1001794789
...
//uint8_t MyIpAddress[4] = {192, 168, 1, 80};
        ...
        else
        {
            ...
            if (strcmp(token, "set") == 0)
            {
                ...
                if (strcmp(token, "mqtt") == 0)
                {
                    ...
                    setIpMqttBrokerAddress(ip);
                    enableTcp();
                    // getIpMqttBrokerAddress() before send ARP request
                    // Find socket with this IP, send ARP request, process response and set Socket, then start TCP handshake
                    //sendArpRequest() -> isArpResponse -> processResponse
                    p32 = (uint32_t*)ip;
...
int main(void)
{
    ...
    setEtherMacAddress(2, 3, 4, 5, 6, 115);
